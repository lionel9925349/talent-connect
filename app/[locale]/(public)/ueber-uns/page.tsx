            ...
            <div className="grid grid-cols-2 gap-4">
              {stats.map((stat) => (
                <Card key={stat.label} className="text-center">
                  <p className="text-4xl font-bold text-accent" style={{ fontFamily: 'var(--font-heading)' }}>
                    {stat.value}
                  </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {team.map((member) => (
              <Card key={member.name} hover>
                <div className="w-16 h-16 bg-primary rounded-full flex items-center justify-center text-white text-2xl font-bold mb-4" style={{ fontFamily: 'var(--font-heading)' }}>
                  {member.name[0]}
                </div>
                <h3 className="font-bold text-foreground" style={{ fontFamily: 'var(--font-heading)' }}>{member.name}</h3>
